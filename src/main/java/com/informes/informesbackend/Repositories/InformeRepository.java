...
public interface InformeRepository extends JpaRepository<InformeDesempenio, Long> {
    ...
    @Query(value = "SELECT count(*) FROM informes_desempenio where asignatura_id in( select asignatura_id from asignaturas where nombre=:asignatura and anio_curso=:anio and ciclo_lectivo=:cicloLectivo);", nativeQuery=true)
    int findNumInformeByAsignatura( String asignatura, String anio, String cicloLectivo);
