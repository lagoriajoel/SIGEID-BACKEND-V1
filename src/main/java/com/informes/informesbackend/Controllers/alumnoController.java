...
public class alumnoController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @PutMapping("/agregarCurso/{id}/cursoId/{idCurso}")
    public ResponseEntity<?> agregarCurso(@PathVariable Long id, @PathVariable Long idCurso) {

         Optional<Curso> cursoOptional=cursoService.porId(idCurso);
       ...
       Set<Curso> cursos= new HashSet<>();
        ...
        cursos=alumnoOptional.get().getCurso();
        cursos.add(nuevoCurso);
        alumnoOptional.get().setCurso(cursos);

        service.guardar(alumnoOptional.get());
